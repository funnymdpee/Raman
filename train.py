...
def custom(model,model_dir,epochs = 100):
    ...
    resume_path = ckpt_dir+"/rrcd_epoch058.pt"  # <-- 如果不恢复就设 None
    #resume_path = None
    start_epoch = 1

    scheduler = torch.optim.lr_scheduler.StepLR(opt, step_size=200, gamma=0.1)
...
if __name__ == "__main__":
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = create_LKDNet().to(device)
    custom(model=model,model_dir="lkdnet")
